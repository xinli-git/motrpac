...
rna_seqc_script=~/projects/kp1/rna_seqc/rna_seqc_kp1.sh
...
awk -v star_bam_output=$star_bam_output -v rnaqc_output=${rna_seqc_output} 'BEGIN{OFS="\t"}{print star_bam_output"/"$1".out.bam", rnaqc_output"/"$1}' | \
parallel --jobs 24 --col-sep "\t" "${rna_seqc_script} {1} {2}"
